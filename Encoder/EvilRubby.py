...
""" + self.RESET)
    ...
    def compile(self, arguments):
        ...
        # Let the games begin. Reading the input file and handling each
            ...
            if command != 'REM':
                ...
                # CTRL-ALT keys combination
                    ...
                    if arg:
                        ...
                        bytectrlalt = hex(
                            (int(self.props["MODIFIERKEY_CTRL"], 16)) + (int(self.props["MODIFIERKEY_ALT"], 16)))
                ...
                # CTRL-SHIFT keys combination
                    ...
                    if arg:
                        ...
                        # Add values from each key to get the resultant byte
                        bytectrlalt = hex(
                            (int(self.props["MODIFIERKEY_CTRL"], 16)) + (int(self.props["MODIFIERKEY_SHIFT"], 16)))
                        # Format the resultant value to 0xHH
                        bytectrlalt = ("0x{:02x}".format(int(bytectrlalt, 16)))
                        self.__addByte([bytectrlalt,])
                ...
                # COMMAND-OPTION keys combination (Mac OS)
                    ...
                    if arg:
                        ...
                        # Add values from each key to get the resultant byte
                        bytectrlalt = hex(
                            (int(self.props["MODIFIERKEY_LEFT_GUI"], 16)) + (int(self.props["MODIFIERKEY_ALT"], 16)))
                        # Format the resultant value to 0xHH
                        bytectrlalt = ("0x{:02x}".format(int(bytectrlalt, 16)))
                        self.__addByte([bytectrlalt,])
                ...
                # ALT-SHIFT keys combination
                    ...
                    if arg:
                        ...
                        # Add values from each key to get the resultant byte
                        bytectrlalt = hex(
                            (int(self.props["MODIFIERKEY_LEFT_ALT"], 16)) + (int(self.props["MODIFIERKEY_SHIFT"], 16)))
                        # Format the resultant value to 0xHH
                        bytectrlalt = ("0x{:02x}".format(int(bytectrlalt, 16)))
                        self.__addByte([bytectrlalt,])
                    else:
                ...
                if '//' not in line:
                    ...
                    if " " in line:
                        ...
                        if current_ins[0] not in instruntions_dic:
                            print("Instruction not found " + line.strip())
                            continue
                    else:
                    ...
                    if current_ins[0] == "REPEAT":
                        ...
                        if delay != -1:
                            instructions.append(["DELAY", delay])
                        # Keep the previous instruction in case we need to
                        # repeat it
    ...
    def __loadProperties(self, lang):
        ...
        # Check if the default properties file exists and if we can open it in
        # read mode
                ...
                for line in file_read:
                    if line != '\n':
                        if line[:2] != '//':
                            ...
                            props_def.update(
                                dict(zip(temp_list[0::2], temp_list[1::2])))
            except IOError:
        ...
        # Check if the keyboard layout properties file exists and if we can
                        ...
                        if '//' not in line:
                            ...
                            if ',' in temp_list[1]:
                                ...
                                for i in range(len(temp_list[1])):
                                    temp_list[1][i] = props_def[temp_list[1]
                                                                [i].strip()]
                            else:
